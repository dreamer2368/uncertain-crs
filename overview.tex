...
\section{Step-wise ionization cross section}
\begin{itemize}
\item TRINITI dataset in LXCat provides the meta-stable ionization cross section.
Its reference~\cite{Dixon1976}, however, is about that of helium, not argon.
We suspect that the data is obtained by the same methodology, however the detail is not known.
\item Dixon et al.\cite{Dixon1976}\\
Total ionization cross section is measured by intersecting a helium atom beam and a mono-energetic electron beam.
The atom beam goes through a ion deflection before intersection in order to filter out existing ion population.
An estimation procedure is required to subtract contributions from ground state ionization and other higher-level step-wise ionizations.
While the uncertainty for total ionization cross section is provided as maximum $6\%$,
that for step-wise ionization itself is not specified, only suspected to be larger.
\item \cite{Ton-That1977}\\
This paper includes an experimental data of Dixon's group, via private communication.
We suspect that the same methodology in \cite{Dixon1976} is used.
The data is not identical to TRINITI dataset, but agrees well with it.
Its uncertainty is not specified.
\end{itemize}
